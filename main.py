...
def delete_usuario(usuario_id: int, db: Session = Depends(get_db)):
    usuario = db.query(Usuario).filter(Usuario.id == usuario_id).first()
    if not usuario:from fastapi import FastAPI, Depends, HTTPException, status
from sqlalchemy.orm import Session
from sqlalchemy import Column, Integer, String
from database import SessionLocal, engine
from models import Base, Usuario
...
class UsuarioCreate(BaseModel):
    ...
    email: str
    lastname: str
    apellido: str  # "apellido" usado como la credencial en lugar de la contraseña

# Pydantic model para la autenticación
class UsuarioLogin(BaseModel):
    email: str
    apellido: str  # Validamos usando apellido como credencial
...
# Función para verificar la contraseña (no se usa en este caso, pero se mantiene por consistencia)
def verify_password(plain_password, hashed_password):
    return pwd_context.verify(plain_password, hashed_password)
...
def create_usuario(usuario: UsuarioCreate, db: Session = Depends(get_db)):
    ...
    db_usuario = Usuario(
        ...
        apellido=usuario.apellido  # Usar apellido como la credencial
    )
...
def login(usuario: UsuarioLogin, db: Session = Depends(get_db)):
    db_usuario = db.query(Usuario).filter(Usuario.email == usuario.email).first()
    if not db_usuario or db_usuario.apellido != usuario.apellido:  # Validar usando apellido
        raise HTTPException(status_code=401, detail="Credenciales inválidas")
